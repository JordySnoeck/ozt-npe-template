...
Hier wordt de variabele **SecondaryEducation** (door `mutate()`) gewijzigd in een ordinale variabele (door `factor()`) met geordende niveaus (optie `levels`). Je kan de verschillende mogelijkheden die er zijn opvragen via de functie `unique()`, bv:
...
Hieronder volgt een uitgewerkt voorbeeld. De toetsresultaten zijn samengevat per niveau (lager, 1e en 2e graad secundair) in de variabelen **TestLevel1**, **2** en **3** en ook het totaal is berekende in **TestTotal**. Verder zijn ook de variabelen **EducationFather** en **SecondaryEducation** gesorteerd omdat het ordinale variabelen zijn.
